<template>
  <ul class="select">
    <li class="select-item" v-for="item of selectData">
      <ul>
        ...
        <slot name="select-custom-content">
          ...
          <template v-else>
            <li class="select-item-option" v-for="option of item.options" @click="onSelect(option)">
              <span>
                {{ option.name }}
              </span>
              <CloseOutlined :key="option" class="select-item-del" v-show="item.hasDel" />
            </li>
          </template>
        </slot>
      </ul>
